...
const user = {
    ...
    mutations: {
        CHANGE_PWD: (state, value) => {
            state.changePwd = value
        },
        ...
        GetInfo({ commit, state }) {
            return new Promise((resolve, reject) => {
                getInfo(state.token).then(res => {
                    const user = res.user
                    const avatar = user.avatar && user.avatar !== "" ? getFileUrl(user.avatar) : require("@/assets/image/psn.jpg") ;
