<template>
  <div class="app-container">
    ...
    <!-- 分配角色数据权限对话框 -->
    <el-dialog v-dialogDrag append-to-body :title="title" :visible.sync="openDataScope" width="500px">
...
export default {
  ...
  methods: {
    ...
    getRoleOrgTreeselect(roleId) {
      roleOrgTreeselect(roleId).then(response => {
        this.$refs.org.setCheckedKeys(response.data);
      });
    },
    /** 根据角色ID查询部门树结构 */
    ...
    handleStatusChange(row) {
      ...
      this.$confirm(
        ...
        .then(function() {
          return changeRoleStatus(row.roleId, row.status, row.version);
        })
        .then(retData => {
    ...
    handleUpdate(row) {
      ...
      getRole(roleId).then(response => {
        ...
        this.$nextTick(() => {
          ...
          this.getRoleOrgTreeselect(roleId);
      });
